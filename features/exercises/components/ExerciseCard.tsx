    ...
    >
      ...
      <div className="p-6 pl-7 flex flex-col h-full">
        <div className="flex items-start justify-between mb-5">
          <div>
            <h4 className="text-xl font-bold uppercase text-white group-hover:text-primary transition-colors tracking-tight leading-tight">
              {exercise.name}
            </h4>
        ...
        {/* Stats Grid */}
        <div className="grid grid-cols-3 gap-0 mb-6 border-y border-border-dark/30 py-5 bg-black/10 -mx-6 px-6">
          <div className="flex flex-col border-r border-border-dark/30 pr-2">
            <span className="text-[9px] uppercase font-mono text-text-muted tracking-[0.2em] mb-2 font-bold">Anterior</span>
            <div className="flex items-baseline">
              <span className="text-2xl font-mono font-bold text-white tracking-tighter leading-none">
                {exercise.lastWeight}
              </span>
              <span className="text-[10px] font-mono text-text-muted uppercase ml-0.5">kg</span>
            </div>
            <span className="text-[10px] text-text-muted font-mono mt-1 opacity-60 uppercase">{exercise.lastDate}</span>
          </div>

          <div className="flex flex-col border-r border-border-dark/30 px-4">
            <span className="text-[9px] uppercase font-mono text-primary tracking-[0.2em] mb-2 font-bold">RP</span>
            <div className="flex items-baseline">
              <span className="text-2xl font-mono font-bold text-primary tracking-tighter leading-none drop-shadow-[0_0_8px_rgba(0,240,255,0.3)]">
                {exercise.pbWeight}
              </span>
              <span className="text-[10px] font-mono text-primary/70 uppercase ml-0.5 font-bold">kg</span>
            </div>
            <span className="text-[10px] text-text-muted font-mono mt-1 opacity-60 uppercase">{exercise.pbDate}</span>
          </div>

          <div className="flex flex-col pl-4">
            <span className="text-[9px] uppercase font-mono text-text-muted tracking-[0.2em] mb-2 font-bold">Vol. Méd.</span>
            <div className="flex items-baseline">
              <span className="text-2xl font-mono font-bold text-white tracking-tighter leading-none">
                {exercise.avgVolume}
              </span>
              <span className="text-[10px] font-mono text-text-muted uppercase ml-0.5">k</span>
            </div>
            <span className="text-[10px] text-text-muted font-mono mt-1 opacity-60 uppercase tracking-tighter">Semanal</span>
          </div>
        </div>
          ...
          <div className="border-t border-border-dark/30 pt-4 flex flex-col gap-2">
            ...
            {exercise.history && exercise.history.length > 0 ? (
              exercise.history.map((log, idx) => (
                <div key={idx} className="flex items-center justify-between bg-black/20 p-2 border border-border-dark/20">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-mono text-text-light">{log.weight}kg</span>
                  </div>
